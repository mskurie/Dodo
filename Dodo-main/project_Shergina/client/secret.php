...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Мотивационные сообщения для всех пользователей
    $motivation_messages = [
        "💪 Ты прекрасен(на) таким(ой), какой(ая) есть!",
        "🌟 Каждый день — это новый шанс стать лучше!",
        "😊 Здоровье и счастье — вот что действительно важно!",
        "✨ Ты уникален(на), и это твоя суперсила!",
        "💖 Люби себя и своё тело — оно у тебя одно!",
        "🍕 Пицца — это радость, а радость должна быть в удовольствие!",
        "🌱 Каждый день — маленький шаг к лучшей версии себя.",
        "💫 Ты способен(на) на великие дела, главное — верить в себя!",
        "🌸 Будь добр(а) к себе, ты это заслуживаешь!",
        "🎉 Жизнь прекрасна, а ты — её главный герой!"
    ];
    $motivation_message = $motivation_messages[array_rand($motivation_messages)];
    
    // Добрые комментарии в зависимости от результата
    if ($pizzas_per_day < 1) {
        $comments = [
            "🍕 Ты очень сдержанный(ая) ценитель(ница) пиццы! Умеренность — ключ к гармонии.",
            "🥗 Здоровое питание — это здорово! Но иногда можно побаловать себя вкусненьким.",
            "🤏 Одна пицца может растянуться на несколько дней — отличная экономия и контроль!",
            "🧘‍♂️ Твой подход к питанию заслуживает уважения! Баланс — это важно.",
            "🍕 Даже маленький кусочек пиццы может сделать день ярче и счастливее!"
        ];
    } elseif ($pizzas_per_day < 2) {
        $comments = [
            "🍕 Одна пицца в день — идеальный баланс для счастливой и здоровой жизни!",
            "😎 У тебя отличное чувство меры! Так держать!",
            "🎯 Ты нашёл(ла) золотую середину между вкусом и пользой!",
            "👌 Стабильность и умеренность — признаки мудрости! Продолжай в том же духе.",
            "💪 Умеренность во всём — ключ к долголетию и отличному самочувствию!"
        ];
    } elseif ($pizzas_per_day < 3) {
        $comments = [
            "🍕🍕 Отличный аппетит! Главное — наслаждаться каждым кусочком с удовольствием!",
            "🔥 Настоящий(ая) ценитель(ница) пиццы! Помни про активный образ жизни.",
            "💪 Твой организм знает, что ему нужно для счастья! Добавь немного движения — и будет идеально.",
            "🎉 Жизнь хороша, когда в ней есть место любимой еде и радости!",
            "🍕 Две пиццы — это отличный повод пригласить друга и разделить радость трапезы!"
        ];
    } else {
        $comments = [
            "🍕🍕🍕 Вау! Ты настоящий(ая) любитель(ница) пиццы с большим сердцем! 🎉",
            "🚀 У тебя невероятная энергия и жизнелюбие! Не забывай про активный отдых и прогулки.",
            "🏆 Твой энтузиазм и любовь к жизни достойны восхищения! Живи в удовольствие!",
            "🌟 Ты умеешь радоваться жизни — это прекрасный дар!",
            "🎪 С тобой всегда весело и интересно! Помни, что движение — это тоже радость и здоровье.",
            "🍕 Любовь к пицце — это любовь к жизни во всех её проявлениях! Наслаждайся каждым моментом!"
        ];
    }
...
<head>
    ...
    <title>🍕 Калькулятор пиццы · Додо Пицца</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Inter', sans-serif; }
        ...
        .motivation-banner { background: linear-gradient(135deg, #FFD700 0%, #FFA500 100%); color: #1A1A1A; padding: 20px 25px; border-radius: 20px; text-align: center; margin-bottom: 25px; box-shadow: 0 4px 16px rgba(255,215,0,0.3); }
        .motivation-text { font-size: 1.2rem; font-weight: 700; line-height: 1.5; }
        ...
        .comment-text { font-size: 1.1rem; font-weight: 500; color: #1A1A1A; line-height: 1.5; }
...
<body>
    ...
    <div class="hero">
        <div class="container">
            ...
            <h1>Калькулятор пиццы</h1>
            <p>Узнай, сколько пиццы ты можешь съесть за день<br>и оставаться в гармонии с собой! 😊</p>
        </div>
    </div>
    ...
    <div class="container">
        ...
        <?php if ($result): ?>
            <div class="result-card">
                <div class="motivation-banner">
                    <div class="motivation-text">✨ <?php echo $motivation_message; ?> ✨</div>
                </div>
                ...
                <div class="pizza-result">
                    ...
                    <div class="pizza-label">пиццы в день (примерно)</div>
                </div>
        ...
        <p class="footer-note">🤫 Это секретная страница! Наслаждайся пиццей с удовольствием! 🍕</p>
    </div>
</body>
</html>
...
if (!$menu_result) {
    ...
    $menu = $menu_result->fetch_all(MYSQLI_ASSOC);
}
    ...
    <div class="container">
        <?php if (empty($menu_by_category)): ?>
            <div style="text-align: center; padding: 60px; background: white; border-radius: 24px;">
                <p style="font-size: 1.2rem; color: #757575;">Меню временно недоступно. Пожалуйста, зайдите позже.</p>
            </div>
        <?php else: ?>
                        ...
                        <?php foreach ($dishes as $dish): ?>
                            <div class="dish-card">
                                <?php if (!empty($dish['image'])): ?>
                                    <img src="../uploads/dishes/<?php echo htmlspecialchars($dish['image']); ?>"
                                         alt="<?php echo htmlspecialchars($dish['name']); ?>" class="dish-image">
                                <?php else: ?>
